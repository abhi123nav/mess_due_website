...
<body>
    ...
    <div class="container">
<form  method="GET">                                                          <!r can here only once put method="GET"*/!>
    <h4> MESS DUE</h4><br>
    <div class="textbox">
         NAME  : <input class="textbox"   size="20px" type="text" name="name" action="mess1.php" method="GET" placeholder="NAME"><br><br><br>
          ROLL NO : <input class="textbox"  size="20px" type="text" name="rollno" action="mess1.php" method="GET" placeholder="ROLL NO" ><br><br><br>
...
{    $name1=$_GET['name'];
 ...
 //convert name to either lower or upper case
    ...
    if(!empty($name)&&!empty($rollno))
    {
     $query ="SELECT id FROM mess1234 WHERE name='".$name."'AND rollno='".$rollno."'";
        
             $result= mysqli_query($w,$query); 
                // echo mysqli_num_rows($result);                                                    
             if(mysqli_num_rows($result)==1)                                                     
              {  
                $sql = "SELECT * from mess1234 WHERE name='".$name."'AND rollno='".$rollno."'";
                $result = mysqli_query($w, $sql);
                while($row = $result->fetch_assoc()) 
                    {
                    echo "NAME : ".$row["name"]."<br>";
